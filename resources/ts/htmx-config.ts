...
export function configureHTMX(): void {
    ...
    document.addEventListener('htmx:responseError', (event: any) => {
    ...
    // Manejo de timeout
    document.addEventListener('htmx:timeout', (event: any) => {
 ...
 */
function handleValidationErrors(xhr: XMLHttpRequest): void {
    try {
        const response = JSON.parse(xhr.responseText);
        ...
        if (response.errors) {
            Object.entries(response.errors).forEach(([field, messages]) => {
                ...
                if (fieldElement && Array.isArray(messages)) {
                    showFieldError(fieldElement as HTMLElement, messages[0]);
                }
            });
 ...
 */
function createNotification(type: 'success' | 'error', message: string): void {
