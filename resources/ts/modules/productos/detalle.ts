...
export class ProductoDetalle {
    ...
    private getProductId(): number {
        const hiddenInput = document.querySelector<HTMLInputElement>('input[name="id"]');
    ...
    private validateQuantityInput(input: HTMLInputElement): void {
        input.value = input.value.replace(/[^0-9]/g, '');
    }

    private normalizeQuantity(input: HTMLInputElement): void {
    ...
    private handleQuantityKeydown(e: KeyboardEvent): void {
        if (e.key === 'ArrowUp') {
            e.preventDefault();
    ...
    private animateQuantityButton(action: 'increment' | 'decrement'): void {
    ...
    private async addToCart(): Promise<void> {
        if (!this.addToCartForm || !this.quantityInput) return;
    ...
    private showNotification(message: string, type: 'success' | 'error'): void {
        ...
        notification.className = `fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg transition-all duration-300 ${
            type === 'success' ? 'bg-ive-green text-white' : 'bg-ive-red text-white'
        }`;
